...
procedure copia_conjunto (var c1, c2: conjunto);
(*
...
function retira_um_elemento (var c: conjunto): elemento;
(*
...
var 
	...
	qntdHerois,qntdEquipes : longint;
	herois : array[0..MAX-1] of conjunto;
	habilidadesRequiridas, eqVAI, eqTesta, u: conjunto;
...
*)
    inicializar_conjunto(eqVAI);
    inicializar_conjunto(u);

    writeln('Insira as os herois : ');
	ler_conjunto(herois[1]);
	i := 1;
	...
	qntdHerois :=  i - 1 ;
     writeln('numero de herois : ', qntdHerois);

    writeln('Insira as habilidades requeridas : ');
	ler_conjunto(habilidadesRequiridas);

(*
    writeln('Insira as equipes : ');
	ler_conjunto(equipes[1]);
	i := 1;
	...
	qntdEquipes := i - 1;
    writeln('numero de equipes : ', qntdEquipes);
*)
    
    writeln('Insira as equipes : ');
    ler_conjunto(eqTesta);
    while not eh_vazio(eqTesta) do 
    begin
        for i := 1 to eqTesta[0] do 
        begin
            uniao(herois[eqTesta[i]],u,u);
        end;       
            write('PODERS DO GRUPO : ');
            imprimir_conjunto(u);
        
            if contido(u, habilidadesRequiridas) then
            begin
                writeln('Essa equipe tem todos os poderes necessarios!');
                if (eqTesta[0] < eqVAI[0]) or (eqVAI[0] = 0) then
                begin
                    writeln('E eh melhor que a antiga!');
                    copia_conjunto(eqTesta,eqVAI);
                end;
            end; 
